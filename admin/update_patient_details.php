<?php require_once"db/session.php";
	...
	extract(mysqli_fetch_array(select("select user.*, patient_information.* from  user INNER JOIN patient_information on user.userid=patient_information.student_id WHERE user.userid='".$_REQUEST['patientid']."'")));
?>
<!DOCTYPE HTML>
<html>
	<head>
					...
					<div class="widget-shadow">
						<div class="login-body">
							<form method="post" id="update_details">
								<input type="hidden"  value="<?=$_REQUEST['patientid']?>" id="patientid" readonly>
			...
			$(window).load(function(){
				$("#update_details").submit(function(){
					...
					$.ajax({
						url : "ajax/user.php",
						method:"post",
						data:{"patient_name":patient_name,"patient_email":patient_email,"gender":mygender,"patientid":patientid,"dob":dob,"contact_no":contact_no,"address":address,"desc":desc,"update_patient_details":1},
						success:function(data){
							if(data==1)
							{
								...
								alertify.alert("Patient Details Updated", function(){
