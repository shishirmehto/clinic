<?php require_once"db/session.php";
	...
	extract(mysqli_fetch_array(select("select doctor.*, doctor_information.* from doctor INNER JOIN doctor_information on doctor.doctorid=doctor_information.student_id WHERE doctor.doctorid='".$_SESSION['doctorid']."'")));
?>
<!DOCTYPE HTML>
<html>
	<head>
					...
					<div class="widget-shadow">
						<div class="login-body">
							<form method="post" id="update_details">
								...
								</div><input type="date"  class="user animated rotateInUpLeft" value="<?=$date_of_birth?>" id="dob" placeholder="<?=date("Y-m-d");?>">
			...
			$("#doctor_name").focus();
				$("#update_details").submit(function(){
					...
					$.ajax({
						url : "ajax/user.php",
						method:"post",
						data:{"doctor_name":doctor_name,"gender":mygender,"dob":dob,"contact_no":contact_no,"address":address,"desc":desc,"update_details":1},
